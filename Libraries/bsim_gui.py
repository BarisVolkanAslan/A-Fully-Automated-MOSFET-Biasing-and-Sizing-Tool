...
from Libraries import bsim_ng
from Libraries import bsim_ng_func
from Libraries import bsim_ng_base
from Libraries import start_gui

ng_base  = bsim_ng_base 
ng_func = bsim_ng_func
ng = bsim_ng
...
def define_global_paths():
    ...
    database_path = "Database/program_inputs_bsim.db"
    bsim_sp = "bsim.sp"
...
def initialize():   
    root = tk.Tk()
    root.title("Microelectronics Laboratory : MOSFET Biasing and Sizing Tool - BSIM3v3.2 Model Deck for Standard-Threshold Devices")
    ...
    try:
        ...
        messagebox.showinfo("Information", "BSIM Database file was already created.") 
    ...
    tk.Label(inp_frame, text="Type model deck").grid(row=1, column=0, sticky=tk.E)   #Type model
    ...
    inp_model_deck.insert(0, "bsim.sp")
    ...
    tk.Label(inp_frame, text="").grid(row=1, column=2, sticky=tk.W) #(ex: bsim.sp)
...
def database_edit():
        try:
             ...
             global edt_root
             edt_root = tk.Tk()
             edt_root.title("Update BSIM Record " + record_id)
             ...
             # Create Application Spesifics Text Box Labels
             tk.Label(edt_frame, text="Type model deck").grid(row=4, column=0, sticky=tk.E)
             ...
             # Create Units and Examples
             tk.Label(edt_frame, text="(ex: bsim.sp)").grid(row=4, column=2, sticky=tk.W)
...
def error_checker(model_deck, model, Ldrmin, Wdrmin, Vdsat, gm, gmb, rds, Id, Vsb, Vds, Vgs, Ldr, sim_option_speed):
    ...
    global bsim_speed
    global iter_speed
    bsim_speed, iter_speed = bsim_ng.define_speed_var(sim_option_speed)
    ng_base.bsim_sp(bsim_speed)
    try :
        ...
        error_count += 1
        try:
            os.remove(raw_data_path + bsim_sp)
            os.remove(raw_data_path + bsim_sp)
        except:
            print("can not find bsim.sp")
...
def verification():
        try :
            ...
            try:
                if (Vgs_res < 0):
                    MOS = 0
                    Vdsat, Id, gm, gmb, rds = ng_base.sort_results(p_verify_txt_path, MOS, model_deck, Wdr_res, Ldr_res, Vgs_res, Vds_res, Vsb_res, iter_speed, bsim_speed)
                    Vdsat = -Vdsat
            
                else:
                    MOS = 1
                    Vdsat, Id, gm, gmb, rds = ng_base.sort_results(n_verify_txt_path, MOS, model_deck, Wdr_res, Ldr_res, Vgs_res, Vds_res, Vsb_res, iter_speed, bsim_speed)
    ...
    try:
        ...
        if(error_checker_inp()):
            try: 
                ...
                model_deck, model, Wdrmin, Ldrmin, Vdsat, gm, \
                gmb, rds, Id, Vsb, Vds, Vgs, Ldr )
