...
def define_speed_var(sim_speed):
    ...
    if(sim_speed == 0):
        ...
        bsim_speed = 1
        iter_speed = 1     
    return bsim_speed, iter_speed

"""
Call this function from main.py.
This function will start a bsim.sp simulation.
...
def simulate(sim_speed, sim_option, model_deck, model_name, Wdrmin, Ldrmin, Vdsat, gm, \
    ...
    bsim_speed, iter_speed = define_speed_var(sim_speed)
    ngbase.bsim_sp(bsim_speed)
        
    MOS = ngfunc.whichMOS(model_deck, model_name)
...
def calc_Wdr (W_L, Ldr, model_deck, model_name) :
    LINT = float(ngfunc.extract_bsim_param(model_deck, model_name, "LINT"))
    WINT = float(ngfunc.extract_bsim_param(model_deck, model_name, "WINT"))
    XW = float(ngfunc.extract_bsim_param(model_deck, model_name, "XW"))
    XL = float(ngfunc.extract_bsim_param(model_deck, model_name, "XL"))        
    L = Ldr + XL - (2*LINT)
    W = (W_L)*L
    Wdr = W - XW + (2*WINT)
