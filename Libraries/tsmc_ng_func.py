...
"sim_or_check" differs during the simulation process and check process
checking is done through the original file path,
simulation is being run with a bunch of ngspice .cir files 
this is why the located model_deck is copied to the sim_data folder during
the simulation process. Copied model deck will be deleted afterwards.
10/04/21
"""
def whichMOS(model_deck, model_name, sim_or_check):
    if(sim_or_check == 0):
...
"""  Extract parameters from a TSMC model deck"""    
def extract_tsmc_param(model_deck, model_name, param_name):
    model_name = model_name.upper()
