...
def initialize():   
    root = tk.Tk()
    root.title("Microelectronics Laboratory : MOSFET Biasing and Sizing Tool - UMC Model Deck")
    root.iconbitmap(icon_path)
    root.geometry("1480x659")
    ...
    try:
        ...
        messagebox.showinfo("Information", "UMC Database file was already created.") 
    ...
    """
    global inp_frame
    inp_frame = tk.LabelFrame(root, text="DATA INPUTS", padx=10, pady=16, bd = 5, labelanchor = 'n', font = "none 15 bold")
    inp_frame.grid(column=0, row=0, padx=10, pady=20) 
    

    # APPLICATION SPESIFICS FRAME
    tk.Label(inp_frame, text="APPLICATION SPESIFICS", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(column=1, row=1)
    # Create Application Spesifics Text Box Labels
    tk.Label(inp_frame, text="Type model deck").grid(row=2, column=0, sticky=tk.E)
    tk.Label(inp_frame, text="Type .lib").grid(row=3, column=0, sticky=tk.E)
    tk.Label(inp_frame, text="Type .model").grid(row=4, column=0, sticky=tk.E)
    # Create Application Spesifics Text Boxes
    global inp_model_deck
    global inp_lib_name
    global inp_model
    inp_model_deck = tk.Entry(inp_frame, width=30)
    inp_model_deck.grid(row=2, column=1, padx=20)
    inp_model_deck.insert(0, "Locate a UMC model deck file.")
    inp_model_deck.configure(state= tk.DISABLED)
    
    
    inp_lib_name = tk.Entry(inp_frame, width=30)
    inp_lib_name.grid(row=3, column=1, padx=20) 
    inp_model = tk.Entry(inp_frame, width=30)
    inp_model.grid(row=4, column=1, padx=20)
    ...
    tk.Button(inp_frame, image=open_btn, command=open_file, borderwidth=0).grid(row=2, column=2, sticky=tk.W)
    tk.Label(inp_frame, text="(ex: tt)").grid(row=3, column=2, sticky=tk.W)
    tk.Label(inp_frame, text="(ex: n_lv_18_mm)").grid(row=4, column=2, sticky=tk.W)
   ...
   # INITIAL GUESS FRAME    
    ...
    tk.Label(inp_frame, text="Type initial Ldr").grid(row=19, column=0, sticky=tk.E)
    ...
    # INPUT BUTTONS
    # Create Info Button
    tk.Button(inp_frame, text="Information", command=info_button).grid(row=21, column=0, columnspan=3, pady=6, padx=10, ipadx=145)
    # Create Clear Button
    ...
    database_frame = tk.LabelFrame(Simulation_frame, 
                                     padx=38, pady=10, bd = 0,text="CONFIGURE INPUT DATABASE",
    ...
    ############################
    """
    out_frame = tk.LabelFrame(root, text="DATA OUTPUTS", padx=35, pady=40, bd = 5, labelanchor = 'n', font = "none 15 bold")
    out_frame.grid(column=2, row=0, padx=5, pady=20)
    
    
    # CREATE APPLICATION SPESIFICS FRAME
    tk.Label(out_frame, text="MODEL DECK VALUES", bd = 2, font = "none 9 bold", anchor=tk.S, padx=5, pady=5).grid(column=1, row=0) 
    # Create Text Box Labels
    tk.Label(out_frame, text="Model").grid(row=1, column=0, sticky=tk.E)
    tk.Label(out_frame, text="LINT").grid(row=2, column=0, sticky=tk.E)
    tk.Label(out_frame, text="WINT").grid(row=3, column=0, sticky=tk.E)
    tk.Label(out_frame, text="XL").grid(row=4, column=0, sticky=tk.E)
    tk.Label(out_frame, text="XW").grid(row=5, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Ldr(min)").grid(row=6, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Ldr(max)").grid(row=7, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Wdr(min)").grid(row=8, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Wdr(max)").grid(row=9, column=0, sticky=tk.E)
    # Create Text Boxes
    global out_model_desc
    global out_LINT
    global out_WINT
    global out_XL
    global out_XW
    ...
    global out_Wdrmin
    global out_Wdrmax
    out_model_desc = tk.Entry(out_frame, width=30)
    out_model_desc.grid(row=1, column=1, padx=5)
    out_LINT = tk.Entry(out_frame, width=30)
    out_LINT.grid(row=2, column=1, padx=5)
    out_WINT = tk.Entry(out_frame, width=30)
    out_WINT.grid(row=3, column=1, padx=5)
    out_XL = tk.Entry(out_frame, width=30)
    out_XL.grid(row=4, column=1, padx=5)
    out_XW = tk.Entry(out_frame, width=30)
    out_XW.grid(row=5, column=1, padx=5)
    out_Ldrmin = tk.Entry(out_frame, width=30)
    out_Ldrmin.grid(row=6, column=1, padx=5)
    out_Ldrmax = tk.Entry(out_frame, width=30)
    out_Ldrmax.grid(row=7, column=1, padx=5)
    out_Wdrmin = tk.Entry(out_frame, width=30)
    out_Wdrmin.grid(row=8, column=1, padx=5)  
    out_Wdrmax = tk.Entry(out_frame, width=30)
    out_Wdrmax.grid(row=9, column=1, padx=5)  
    # Create Units
    tk.Label(out_frame, text="(m)").grid(row=6, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(m)").grid(row=7, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(m)").grid(row=8, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(m)").grid(row=9, column=2, sticky=tk.W)
    tk.Label(out_frame, text=" ").grid(column=1, row=10)



    # CREATE DESIGN VARIABLES FRAME    
    tk.Label(out_frame, text="DESIGN VARIABLES", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(column=1, row=11) 
    # Create Text Box Labels
    tk.Label(out_frame, text="Ldr").grid(row=12, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Wdr").grid(row=13, column=0, sticky=tk.E)
    # Create Text Boxes
    global out_Ldr
    global out_Wdr
    out_Ldr = tk.Entry(out_frame, width=30)
    out_Ldr.grid(row=12, column=1, padx=20)
    out_Wdr = tk.Entry(out_frame, width=30)
    out_Wdr.grid(row=13, column=1, padx=20)
    #Create Units    
    tk.Label(out_frame, text="(\N{MICRO SIGN}m)").grid(row=12, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(\N{MICRO SIGN}m)").grid(row=13, column=2, sticky=tk.W)
    tk.Label(out_frame, text=" ").grid(column=1, row=14)
    
    
    
    # CREATE BIAS VOLTAGES FRAME
    tk.Label(out_frame, text="BIAS VOLTAGES", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(column=1, row=15) 
    # Create Text Box Labels
    tk.Label(out_frame, text="Vsb").grid(row=16, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Vds").grid(row=17, column=0, sticky=tk.E)
    tk.Label(out_frame, text="Vgs").grid(row=18, column=0, sticky=tk.E)
    # Create Text Boxes
    global out_Vsb
    global out_Vds
    global out_Vgs
    out_Vsb = tk.Entry(out_frame, width=30)
    out_Vsb.grid(row=16, column=1, padx=20)
    out_Vds = tk.Entry(out_frame, width=30)
    out_Vds.grid(row=17, column=1, padx=20)
    out_Vgs = tk.Entry(out_frame, width=30)
    out_Vgs.grid(row=18, column=1, padx=20)
    #Create Units    
    tk.Label(out_frame, text="(V)").grid(row=16, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(V)").grid(row=17, column=2, sticky=tk.W)
    tk.Label(out_frame, text="(V)").grid(row=18, column=2, sticky=tk.W)
    tk.Label(out_frame, text=" ").grid(column=1, row=19)
    
    tk.Button(out_frame, text="Iterations", command=simulation_image).grid(row=20, column=0, columnspan=3, pady=6, padx=10, ipadx=145)
    tk.Button(out_frame, text="Verification", command=verification).grid(row=21, column=0, columnspan=3, pady=6, padx=10, ipadx=140)
...
def database_edit():
    try:
        ...
        record_id = database_select_box.get()
        edt_root = tk.Tk()
        edt_root.title("Update UMC Record: " + record_id)
        edt_root.iconbitmap(icon_path)
        edt_root.geometry("640x740") 
        ...
        tk.Label(edt_frame, text="Declare a simulation name").grid(row=1, column=0, sticky=tk.E)
        ...
        # Create empty line
        tk.Label(edt_frame, text=" ").grid(row=2, column=1)
         
        
        # APPLICATION SPESIFICS FRAME
        tk.Label(edt_frame, text="APPLICATION SPESIFICS", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(row=3, column=1)
        # Create Application Spesifics Text Box Labels
        tk.Label(edt_frame, text="Type model deck").grid(row=4, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type .lib").grid(row=5, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type .model").grid(row=6, column=0, stick = tk.E)
        # Create Application Spesifics Text Boxes
        ...
        edt_model_deck.configure(state= tk.DISABLED)
        edt_lib_name = tk.Entry(edt_frame, width=30)
        edt_lib_name.grid(row=5, column=1, padx=20) 
        edt_model = tk.Entry(edt_frame, width=30)
        edt_model.grid(row=6, column=1, padx=20)  
        # Create Units and Examples
        global open_btn
        img = Image.open(open_icon_path)
        resizedImage = img.resize((25, 25), Image.ANTIALIAS)
        ...
        tk.Label(edt_frame, text="(ex: tt)").grid(row=5, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(ex: n_lv_18_mm)").grid(row=6, column=2, stick = tk.W)
        tk.Label(edt_frame, text=" ").grid(row=7, column=1) 
        
        
    
         
        # PERFORMANCE METRICS FRAME    
        tk.Label(edt_frame, text="PERFORMANCE METRICS", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(row=8, column=1)
        # Create Performance Metrics Text Box Labels
        tk.Label(edt_frame, text="Type Vdsat").grid(row=9, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type gm").grid(row=10, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type gmb").grid(row=11, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type rds").grid(row=12, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type Id").grid(row=13, column=0, stick = tk.E)
        # Create Performance Metrics Text Boxes    
        global edt_Vdsat
        global edt_gm
        global edt_gmb
        ...
        edt_Vdsat.grid(row=9, column=1, padx=20)
        edt_gm = tk.Entry(edt_frame, width=30)
        edt_gm.grid(row=10, column=1, padx=20)
        edt_gmb = tk.Entry(edt_frame, width=30)
        edt_gmb.grid(row=11, column=1, padx=20)
        edt_rds = tk.Entry(edt_frame, width=30)
        edt_rds.grid(row=12, column=1, padx=20)
        edt_Id = tk.Entry(edt_frame, width=30)
        edt_Id.grid(row=13, column=1, padx=20)    
        # Create Units and Examples
        tk.Label(edt_frame, text="(V)").grid(row=9, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(mA/V)").grid(row=10, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(mA/V)").grid(row=11, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(k\u03A9)").grid(row=12, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(\N{MICRO SIGN}A)").grid(row=13, column=2, stick = tk.W)
        tk.Label(edt_frame, text=" ").grid(row=14, column=1) 
         
         
        # BIAS VOLTAGES FRAME
        tk.Label(edt_frame, text="EXTERNALLY IMPOSED BIAS VOLTAGES", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(row=15, column=1)
        # Create Bias Voltages Text Box Labels
        tk.Label(edt_frame, text="Type Vsb").grid(row=16, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type Vds").grid(row=17, column=0, stick = tk.E)
        tk.Label(edt_frame, text="Type Vds").grid(row=18, column=0, stick = tk.E)
        # Create Bias Voltages Text Boxes 
        global edt_Vsb
        global edt_Vds
        global edt_Vgs
        edt_Vsb = tk.Entry(edt_frame, width=30)
        edt_Vsb.grid(row=16, column=1, padx=20)
        edt_Vds = tk.Entry(edt_frame, width=30)
        edt_Vds.grid(row=17, column=1, padx=20)
        edt_Vgs = tk.Entry(edt_frame, width=30)
        edt_Vgs.grid(row=18, column=1, padx=20)
        # Create Units and Examples
        tk.Label(edt_frame, text="(V)").grid(row=16, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(V)").grid(row=17, column=2, stick = tk.W)
        tk.Label(edt_frame, text="(V)").grid(row=18, column=2, stick = tk.W)
        tk.Label(edt_frame, text=" ").grid(row=19, column=1)
                         
        
        # INITIAL GUESS FRAME    
        tk.Label(edt_frame, text="INITIAL GUESS", bd = 2, font = "none 9 bold", anchor=tk.S, padx=20, pady=5).grid(row=20, column=1)
        # Create Initial Guess Text Box Labels
        tk.Label(edt_frame, text="Type initial Ldr").grid(row=21, column=0, stick = tk.E)
        # Create Initial Guess Text Boxes 
        global edt_Ldr
        edt_Ldr = tk.Entry(edt_frame, width=30)
        edt_Ldr.grid(row=21, column=1, padx=20)
        # Create Units and Examples
        tk.Label(edt_frame, text="(\N{MICRO SIGN}m)").grid(row=21, column=2, stick = tk.W)
        
            
        # INPUT BUTTONS
        # Create Info Button
        tk.Button(edt_frame, text="Information", command=info_button).grid(row=22, column=0, columnspan=3, pady=6, padx=10, ipadx=145)
        # Create Check Errors Button
        tk.Button(edt_frame, text="Check", command=error_checker_edt).grid(row=23, column=0, columnspan=3, pady=6, padx=10, ipadx=159)
        ...
        for record in records:
            ...
            edt_model_deck.configure(state= tk.DISABLED)  
            edt_lib_name.insert(0, record[2])
            edt_model.insert(0, record[3])
            
            if record[4] != 0 :
                edt_Vdsat.insert(0, record[4])
            if record[5] != 0 :
                edt_gm.insert(0, record[5])
            if record[6] != 0 :
                edt_gmb.insert(0, record[6])
            if record[7] != 0 :
                edt_rds.insert(0, record[7])
            if record[8] != 0 :
                edt_Id.insert(0, record[8])
            
            if record[9] != 0 :
                edt_Vsb.insert(0, record[9])
            if record[10] != 0 :
                edt_Vds.insert(0, record[10])
            if record[11] != 0 :
                edt_Vgs.insert(0, record[11])
            
            edt_Ldr.insert(0, record[12])
...
# Clear the Text Boxes in the input data interface.
def clear_inp_boxes():
...
# Clear all Entry Boxes in Simulation interface.
def clear_boxes():
    database_name.delete(0, tk.END)
...
def clear_out_boxes():
    out_model_desc.delete(0, tk.END)
    out_LINT.delete(0, tk.END)
    out_WINT.delete(0, tk.END)
    out_XL.delete(0, tk.END)
    out_XW.delete(0, tk.END)
    out_Ldrmin.delete(0, tk.END)
    out_Ldrmax.delete(0, tk.END)
    out_Wdrmin.delete(0, tk.END)
    out_Wdrmax.delete(0, tk.END)
...
def enable_out_boxes():
    out_model_desc.configure(state = tk.NORMAL)
    ...
    out_Ldrmin.configure(state = tk.NORMAL)
    out_Ldrmax.configure(state = tk.NORMAL)
    out_Wdrmin.configure(state = tk.NORMAL)
    out_Wdrmax.configure(state = tk.NORMAL)
...
def disable_out_boxes():
    out_model_desc.configure(state = tk.DISABLED)
    ...
    out_Ldrmin.configure(state = tk.DISABLED)
    out_Ldrmax.configure(state = tk.DISABLED)
    out_Wdrmin.configure(state = tk.DISABLED)
    out_Wdrmax.configure(state = tk.DISABLED)
...
def database_query():
    ...
    print_records=''
    for record in records:
        print_records += str(record[13]) + "  :  " + str(record[0]) + " " + "\n"
...
def database_get_inp():
    ...
    if(database_select_box.get()):
        ...
        for record in records:
            ...
            inp_model_deck.configure(state= tk.DISABLED) 
            inp_lib_name.insert(0, record[2])
            inp_model.insert(0, record[3])
            
            if record[4] != 0 :
                inp_Vdsat.insert(0, record[4])
            if record[5] != 0 :
                inp_gm.insert(0, record[5])
            if record[6] != 0 :
                inp_gmb.insert(0, record[6])
            if record[7] != 0 :
                inp_rds.insert(0, record[7])
            if record[8] != 0 :
                inp_Id.insert(0, record[8])
            
            if record[9] != 0 :
                inp_Vsb.insert(0, record[9])
            if record[10] != 0 :
                inp_Vds.insert(0, record[10])
            if record[11] != 0 :
                inp_Vgs.insert(0, record[11])
            
            inp_Ldr.insert(0, record[12])
    else:
        messagebox.showwarning("Warning", "Select a valid record ID")
...
def database_delete():
    ...
    if(database_select_box.get()):
        ...
        messagebox.showwarning("Warning", "Selected a valid record ID.")
...
def error_checker_inp():      
    try:
        model_deck = inp_model_deck.get()
        if ((model_deck != "") or (model_deck != "Locate a UMC model deck file.")):
            try:
                model_lib = inp_lib_name.get()
                try:
                    model = inp_model.get()
                ...
                messagebox.showwarning("Warning", "Please type a lib name.")
        else:
       ...
       return error_checker(model_deck, model_lib, model, Vdsat, gm, gmb, rds, Id, Vsb, Vds, Vgs, Ldr, sim_option_speed)       
def error_checker_edt():
    try:
        model_deck = inp_model_deck.get()
        if ((model_deck != "") or (model_deck != "Locate a TSMC model deck file.")):
            try:
                model_lib = edt_lib_name.get()
                try:
                    model = edt_model.get()
                    try:
        ...
        error_checker(model_deck, model_lib, model, Vdsat, gm, gmb, rds, Id, Vsb, Vds, Vgs, Ldr)        
def error_checker(model_deck, model_lib, model, Vdsat, gm, gmb, rds, Id, Vsb, Vds, Vgs, Ldr, sim_option_speed):
    error_count = 0
    error_message = ''
    global iter_speed
    iter_speed = umc_ng.define_speed_var(sim_option_speed)
    try :
        ...
        if(not (((data_raw[0][0] == model) or (data_raw[0][1] == model)) or ((data_raw[1][0] == model) or (data_raw[1][1] == model)))) :
            ...
            MOS = ng_func.whichMOS(model_deck, model)     
            if (MOS == 1) :
                if (Vds < 0) :
                    error_count += 1
            ...
            Ldrmin = ng_base.extract_umc_param(model_deck, model_lib, model, "lmin")
            if (Ldr < Ldrmin):
                error_count += 1
...
def verification():    
    ...
    messagebox.showinfo("Info", "Verification completed.")
    ...
    try:
        ...
        try :
            # if(error_checker_inp()):
                # try:                
            model_desc, LINT, WINT, XL, XW, \
            Wdrmin, Ldrmin, Wdrmax, Ldrmax, Ldr, Wdr, Vsb, Vds, Vgs \
                    = ng.simulate (sim_option_speed, sim_option,\
            model_deck, model_lib, model, Vdsat, gm, \
            gmb, rds, Id, Vsb, Vds, Vgs, Ldr )
        
            enable_out_boxes()
            
            out_model_desc.insert(0, str(model_desc))
            out_LINT.insert(0, str(LINT).upper())
            out_WINT.insert(0, str(WINT).upper())
            out_XL.insert(0, str(XL).upper())
            out_XW.insert(0, str(XW).upper())
            out_Ldrmin.insert(0, "{:.2f}".format(Ldrmin))
            out_Ldrmax.insert(0, "{:.2f}".format(Ldrmax))
            out_Wdrmin.insert(0, "{:.2f}".format(Wdrmin))
            out_Wdrmax.insert(0, "{:.2f}".format(Wdrmax))
            ...
            model_deck = (model_deck.split("/")[-1])
            
            model_desc = ng_base.extract_umc_description(model_deck, model_lib, model)       
            model_desc = model_desc.split(",")
            
            LINT = ng_base.extract_umc_param(model_deck, model_lib, model, "lint")
            WINT = ng_base.extract_umc_param(model_deck, model_lib, model, "wint")
            XL = ng_base.extract_umc_param(model_deck, model_lib, model, "xl")
            XW = ng_base.extract_umc_param(model_deck, model_lib, model, "xw")
            Ldrmin = ng_base.extract_umc_param(model_deck, model_lib, model, "lmin")
            Ldrmax = ng_base.extract_umc_param(model_deck, model_lib, model, "lmax")
            Wdrmin = ng_base.extract_umc_param(model_deck, model_lib, model, "wmin")
            Wdrmax = ng_base.extract_umc_param(model_deck, model_lib, model, "wmax")
            
            
            os.remove(raw_data_path + model_deck)
            
            enable_out_boxes()
            
            out_model_desc.insert(0, str(model_desc[1]))
            out_LINT.insert(0, str(LINT).upper())
            out_WINT.insert(0, str(WINT).upper())
            out_XL.insert(0, str(XL).upper())
            out_XW.insert(0, str(XW).upper())
            out_Ldrmin.insert(0, str(Ldrmin).upper())
            out_Ldrmax.insert(0, str(Ldrmax).upper())
            out_Wdrmin.insert(0, str(Wdrmin).upper())
            out_Wdrmax.insert(0, str(Wdrmax).upper())
            disable_out_boxes()
