...
def define_global_paths():
    ...
    global model_folder_umc
    raw_data_path = "sim_data/UMC/"
    model_folder_umc = "Model Decks/UMC Model Decks/"
...
"""
def run_circuit(netlist, netlist_path, iter_speed) :
...
def n_mos1_sp(model_deck, Vds, Vsb, iter_speed) :    
    ...
    run_circuit(netlist_new, n_mos1_cir_path, iter_speed)
...
def p_mos1_sp(model_deck, Vds, Vsb, iter_speed) :       
    ...
    run_circuit(netlist_new, p_mos1_cir_path, iter_speed)
...
def n_Va_sp(model_deck, Vsb, Vds, Vgs, Wdr, Ldr, iter_speed) : 
    ...
    run_circuit(netlist_new, n_Va_cir_path, iter_speed) 
...
def p_Va_sp(model_deck, Vsb, Vds, Vgs, Wdr, Ldr, iter_speed) : 
    ...
    run_circuit(netlist_new, p_Va_cir_path, iter_speed)
...
def n_Vth_sp(model_deck, Wdr, Ldr, Vds, Vsb, iter_speed) :
    ...
    run_circuit(netlist_new, n_Vth_cir_path, iter_speed)
...
def p_Vth_sp(model_deck, Wdr, Ldr, Vds, Vsb, iter_speed) :
    ...
    run_circuit(netlist_new, p_Vth_cir_path, iter_speed)
...
def p_verify_sp(model_deck, Wdr, Ldr, Vgs, Vds, Vsb, iter_speed) :
    ...
    run_circuit(netlist_new, p_verify_cir_path, iter_speed)
...
def n_verify_sp(model_deck, Wdr, Ldr, Vgs, Vds, Vsb, iter_speed) :
    ...
    run_circuit(netlist_new, n_verify_cir_path, iter_speed)
...
"""  UMC Model Deck Parameter Extraction
To save time first sort the PDK file which contains a string.
Returns a sorted Raw Data formatted as nested lists.

First lists are seperated by .lib extensions written in pdk file.
Second nested lists are seperated by first lists .model extensions.
Third nested lists are seperated by second lists parameter values.

.lib -> .model -> value

Note that a model_deck must also contain the format of the file.
(04/04/2021)
"""
def extract_umc_param(model_deck, lib_name, model_name, param_name):
    lib_name = lib_name.upper()
    model_name = model_name.upper()
    param_name = param_name.upper()
        
    with open(raw_data_path + model_deck, 'r') as fh:
        data_raw = fh.read().upper().replace(".PARAM", "").replace(".ENDL" , " ").replace("\n", "").replace("\t", " ").replace("+", " ") \
            .replace("=", " ").strip().split(".LIB")
        while("" in data_raw) :
            data_raw.remove("")
        ...
        for x in range(0, len(data_raw)) :
            for y in range(0, len(data_raw[x])) :
                data_raw[x][y] = data_raw[x][y].strip()
                if(not data_raw[x][y].find(".MODEL")) :    
                    data_raw[x][y] = data_raw[x][y].split(" ")
                    while("" in data_raw[x][y]) :
                        data_raw[x][y].remove("")           
            idx_y = len(data_raw[x])-1
            idx_z = len(data_raw[x][y])-1
            del data_raw[x][idx_y][idx_z]    
            
    
        for x in range(0, len(data_raw)) : 
            if(not data_raw[x][0].find(lib_name)):
                for y in range(0, len(data_raw[x])) :            
                        for z in range(0, len(data_raw[x][y])) :      
                            if(not data_raw[x][y][z].find(model_name)):    
                                a=x
                                b=y
                                break
    for c in range(0, len(data_raw[a][b])) :  
        if(not data_raw[a][b][c].find(param_name)):
            value =  data_raw[a][b][c+1]   
            return(value)  


def extract_umc_description(model_deck, lib_name, model_name):
    ...
    with open(raw_data_path + model_deck, 'r') as fh:
        ...
        description = str(data_raw[idx_mn[0]][idx_mn[1]-1])
